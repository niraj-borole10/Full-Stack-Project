...
---
## 📦 Project Structure
├── client/ # Frontend React Application │ ├── src/ │ │ ├── components/ # React Components (EditorPage, Chat, Home) │ │ ├── pages/ # Pages (Login, Register, ForgotPassword) │ │ ├── index.css # Core Tailwind & custom variables theme │ │ └── socket.js # Socket.io Client Setup │ └── package.json │ └── server/ # Backend Node/Express Application ├── config/ # Database configuration (MongoDB Connect) ├── controllers/ # Business Logic Controllers (aiController) ├── routes/ # Router Endpoints (aiRoutes, authRoutes) ├── .env # Server Environment Variables └── index.js # Server Entry Point & Socket events mapping
## ⚙️ Getting Started
### Prerequisites
